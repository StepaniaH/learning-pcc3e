...
for row_death, row_sitka in zip(reader_death, reader_sitka):
    current_date_death = datetime.strptime(row_death[header_row_death.index('DATE')], '%Y-%m-%d')
    current_date_sitka = datetime.strptime(row_sitka[header_row_sitka.index('DATE')], '%Y-%m-%d')
    try:
        high_death = int(row_death[header_row_death.index('TMAX')])
        low_death = int(row_death[header_row_death.index('TMIN')])
        high_sitka = int(row_sitka[header_row_sitka.index('TMAX')])
        low_sitka = int(row_sitka[header_row_sitka.index('TMIN')])
    except ValueError:
...
# Format the plots.
title = f"Daily High and Low Temperatures, 2021\n {row_death[header_row_death.index('NAME')]} and {row_sitka[header_row_sitka.index('NAME')]}"
